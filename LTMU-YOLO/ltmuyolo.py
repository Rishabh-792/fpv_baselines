...
class LTMUYOLO(Tracker):

    def __init__(self):
        super(LTMUYOLO, self).__init__(name='LTMU-YOLO', is_deterministic=False)
    ...
    def init(self, image, box):
        ...
        # YOLO init
        self.yolo_tracker = YOLOv8()
        self.yolo_tracker.init(image, init_gt)
    ...
    def update(self, image):
        ...
        # YOLO track
        self.yolo_tracker.state = [int( self.last_gt[1]), int(self.last_gt[0]), int(self.last_gt[3] - self.last_gt[1]), int(self.last_gt[2] - self.last_gt[0])]
        outputs = self.yolo_tracker.track(image)

        pred_bbox = outputs['pred_bbox']
        local_state = np.array(pred_bbox)
        local_score = np.array(outputs['confidence'])
        ...
        if md_score > 0 and flag == 'normal':
            ...
            if candidate_scores[max_id] > 0:
                ...
                if self.count >= 5:
                    ...
                    self.yolo_tracker.state = [int(redet_bboxes[0]), int(redet_bboxes[1]), int(redet_bboxes[2]), int(redet_bboxes[3])]
